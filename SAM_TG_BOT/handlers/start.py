...
async def process_answer(message: Message, state: FSMContext):
    ...
    question_index = data["current_question"] + 1 
    answer_map = {
        "Juda yaxshi": f"m{question_index}_juda_yaxshi",
        "Yaxshi": f"m{question_index}_yaxshi",
        "O'rtacha": f"m{question_index}_ortacha",
        "Past": f"m{question_index}_past",
        "Yomon": f"m{question_index}_yomon"
    }
    
    score_key = answer_map[answer]
    data["scores"][score_key] = data["scores"].get(score_key, 0) + 1
